	...
	<div class="row">
		...
		<div class="col-sm-6 marginBottom10px"> <?php
			if(isset($_COOKIE["user_type"]) !== "525874964125375325") { ?>
                <a href="<?= base_url('Product/addProductType'); ?>" class="myRingButton pull-right marginLeft10px" style="margin-right:0px;">Add Product Type</a>
                <a href="<?= base_url('Product/addProduct'); ?>" class="myRingButton pull-right marginLeft10px">Add New Product</a> <?php
            } ?>
            <a href="<?= base_url('Product/productInactiveList'); ?>" class="myRingButton pull-right marginLeft10px">Inactive Products</a>
		</div>
	</div>
	<div class="table-container">
		<table id="prduct-table" class="table table-bordered table-striped table-hover">
			...
			<tbody> <?php
				...
				{  ?>
					<form name="updateDiscountForm" method="post" action="<? echo base_url('Product/updateDiscount'); ?>">
						<tr id="tr_<?php echo $product->PROD_ID; ?>">
					
							<td><?php echo $product->PROD_TYPE_NAME; ?></td>
							<td onclick="" title="Go to Cards" style="cursor:pointer">
								<?php echo $product->PROD_NAME; ?>
							</td>
							<td><?php echo $product->NP_SHORT; ?></td>
							<td>
								<input type="text" id="discount" name="discount" class="form-control" value="<?php echo $product->PROD_DISCOUNT; ?>" required size="5" />
							...
							<td>
								<a href="<?php echo base_url('Product/addProduct?ID='.$product->PROD_ID) ?>" title=" Edit Provider"><i class="fa fa-edit fa-2x" aria-hidden="true"></i></a>
							</td>
							<td style="display:none;">
								<input type="checkbox" <?php if($product->PROD_FAV == "1") {  echo "checked"; } ?> id="PROD_FAV" name="PROD_FAV" class="PROD_FAV" value="1"/>
							</td>	
							<td> <?php $prod_id = $product->PROD_ID; ?>
								<input type="hidden" name="id" id="id" class="prod_id" value="<?php echo $product->PROD_ID;?>" />
...
<script type="text/javascript">
    ...
    function filterBy($val){
		if($val == "")
		{
			...
			window.location.assign("productList?filter="+$val);
		}
	}
